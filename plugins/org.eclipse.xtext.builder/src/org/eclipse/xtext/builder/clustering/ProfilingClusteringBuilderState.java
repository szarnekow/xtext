...
public class ProfilingClusteringBuilderState extends ClusteringBuilderState {
	...
	protected Collection<Delta> doUpdate(BuildData buildData, ResourceDescriptionsData newData, IProgressMonitor monitor) {
		...
		LOGGER.info("Doing build (creating global index, linking, validation) took: "
				+ SerializationUtil.milliDiff(start, stop) + " ms");

		return result;
	}

	@Override
	protected void writeNewResourceDescriptions(BuildData buildData, IResourceDescriptions oldState,
			CurrentDescriptions newState, final IProgressMonitor monitor) {
		long start = System.nanoTime();
		super.writeNewResourceDescriptions(buildData, oldState, newState, monitor);
		long stop = System.nanoTime();

		LOGGER.info("Creating new resource descriptions (global scope) took: "
				+ SerializationUtil.milliDiff(start, stop) + " ms");
	}

	@Override
	protected void queueAffectedResources(Set<URI> allRemainingURIs, IResourceDescriptions oldState,
			CurrentDescriptions newState, Collection<Delta> deltas, BuildData buildData, final IProgressMonitor monitor) {
		long start = System.nanoTime();
